/**
 * @file notification_interface.cpp NotificationData class, NotificationInterface class, interface used by clients or
...
std::optional<NotificationData>
NotificationInterface::listen_for_notification(const std::string& id,
...
{ // NOLINT
  ...
  std::optional<NotificationData> msg =
    iomanager::IOManager::get()->get_receiver<NotificationData>(id)->try_receive(std::chrono::milliseconds(timeout));
  ...
  if (msg.has_value()) {
    ...
    if (expected_from != "" && expected_from.find(msg.value().m_source_id) == std::string::npos) {
      TLOG() << "debug : Received request from " << msg->m_source_id << " but expected from " << expected_from
             << " ignoring";
      if (tries <= 1) {
        return std::nullopt;
      }
...
bool
NotificationInterface::send_notification(const notification_type::e_notification_type& notif,
                                         ...
                                         int tries)
{
  ...
  TLOG() << "debug : Sending request " << notification_type::notification_to_string(notif) << " to " << dst << " via "
         << real_conn_id;
