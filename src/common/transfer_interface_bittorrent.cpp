...
char const*
TransferInterfaceBittorrent::state(lt::torrent_status::state_t s)
{
  switch (s) {
...
TransferInterfaceBittorrent::TransferInterfaceBittorrent(GroupMetadata& config,
                                                         bool is_client,
                                                         std::filesystem::path work_dir,
                                                         const IPFormat& listening_ip)
  ...
  , m_thread([&](std::atomic<bool>& running) { this->do_work(running); })
{
...
void
TransferInterfaceBittorrent::do_work(std::atomic<bool>& running_flag)
try {
  bool m_done = false;
    ...
    for (lt::alert const* a : alerts) {
      ...
      if (log_file) {
        std::fprintf(
          log_file,
          "[%ld] %s\n",
          static_cast<std::int64_t>(duration_cast<std::chrono::milliseconds>(a->timestamp() - first_ts).count()),
          a->message().c_str());
      }
      ...
      if (auto p = lt::alert_cast<lt::torrent_finished_alert>(a)) {
        ...
        m_filename_to_metadata[p->torrent_name()]->set_bytes_transferred(
          m_filename_to_metadata[p->torrent_name()]->get_size());
      ...
      if (auto e = lt::alert_cast<lt::peer_error_alert>(a)) {
        ...
        // Try to reconnect
        // lt::error_code ec;
        // h.connect_peer(lt::tcp::endpoint(boost::asio::ip::make_address(config->get_source_ip().get_ip(), ec),
        // std::uint16_t(config->get_source_ip().get_port())));
      ...
      if (auto st = lt::alert_cast<lt::state_update_alert>(a)) {
        ...
        for (uint64_t i = 0; i < st->status.size(); i++) {
          ...
          TLOG() << "is_client " << m_is_client << " [" << i << "]" << s.name << " " << state(s.state) << ' '
                 << (s.download_payload_rate / 1000) << " kB/s " << (s.total_done / 1000) << " kB ("
                 << (s.progress_ppm / 10000) << "%) " << s.current_tracker << " "
                 << static_cast<std::int64_t>(duration_cast<seconds>(s.next_announce).count()) << "s (" << s.num_peers
                 << " peers) " << "\n";
          ...
          //     std::int64_t(duration_cast<seconds>(s.next_announce).count()), s.current_tracker.c_str()); out += str;
          //     pos += 1;
          //     h.post_trackers();
          ...
          //         " : "-  "); out += str; pos += 1; int idx = 0; for (auto const &ep : ae.endpoints)
          //         {
          //             ++idx;
          ...
          //                 idx, v == lt::protocol_version::V1 ? "v1" : "v2", av.fails, ae.fail_limit,
          //                 to_string(int(total_seconds(av.next_announce - now)), 8).c_str(), av.min_announce > now ?
          //                 int(total_seconds(av.min_announce - now)) : 0, av.last_error ?
          //                 av.last_error.message().c_str() : "", av.message.c_str()); out += str; pos += 1;
...
// return magnet url
std::string
TransferInterfaceBittorrent::add_torrent(const std::string& torrent, const std::filesystem::path& dest)
try {
...
void
TransferInterfaceBittorrent::set_torrent_params(lt::add_torrent_params& p, const std::filesystem::path& dest)
{
...
lt::session_params
TransferInterfaceBittorrent::set_settings(const IPFormat& listen_interface, const std::string& listen_port)
{
...
std::string
TransferInterfaceBittorrent::branch_path(std::string const& f)
{
  if (f.empty()) {
    return f;
  }
...
// name starts with a .
bool
TransferInterfaceBittorrent::file_filter(std::string const& f)
{
  if (f.empty()) {
    return false;
  }
...
bool
TransferInterfaceBittorrent::make_torrent(std::filesystem::path full_path,
                                          int piece_size,
...
void
TransferInterfaceBittorrent::generate_torrents_files(const std::filesystem::path& dest, const std::string& tracker)
{
  for (const auto& f_meta : get_transfer_options().get_transfers_meta()) {
    std::filesystem::path tmp = dest;
    make_torrent(f_meta->get_file_path(),
                 static_cast<int>(pow(2, 23)),
                 tracker,
                 tmp.append(f_meta->get_file_name() + ".torrent").string());
  }
}

bool
TransferInterfaceBittorrent::upload_file(TransferMetadata& f_meta)
{
  TLOG() << "debug : uploading " << f_meta.get_file_name();

  if (add_torrent(get_work_dir().append(f_meta.get_file_name() + ".torrent"),
                  f_meta.get_file_path().remove_filename()) == "") {
...
bool
TransferInterfaceBittorrent::download_file(TransferMetadata& f_meta, std::filesystem::path dest)
{
...
bool
TransferInterfaceBittorrent::pause_file(TransferMetadata& f_meta)
{
  ...
  for (const auto& h : handles) {
    if (h.torrent_file()->name() == f_meta.get_file_name()) {
      ...
      TLOG() << "debug : pausing " << f_meta.get_file_name() << " and saving pause data in " << get_work_dir().string()
             << "/.resume_file_" << f_meta.get_file_name();
      break;
    }
  }
...
bool
TransferInterfaceBittorrent::resume_file(TransferMetadata& f_meta)
{
...
bool
TransferInterfaceBittorrent::cancel_file(TransferMetadata& f_meta)
{
  auto const handles = ses.get_torrents();
  for (const auto& h : handles) {
    if (h.torrent_file()->name() == f_meta.get_file_name()) {
...
//     }
//     lt::sha1_hash hash(lt::piece_index_t const piece, lt::span<lt::sha256_hash> const block_hashes, lt::storage_error
//     &ec) const
//     {
...
//     {
//         int const num_pieces = static_cast<int>((m_files.total_size() + m_files.piece_length() - 1) /
//         m_files.piece_length()); return static_cast<int>(piece) < num_pieces - 1
...
//     void async_read(lt::storage_index_t storage, lt::peer_request const &r, std::function<void(lt::disk_buffer_holder
//     block, lt::storage_error const &se)> handler, lt::disk_job_flags_t) override
//     {
...
//     std::shared_ptr<lt::disk_observer>, std::function<void(lt::storage_error const &)> handler, lt::disk_job_flags_t)
//     override
//     {
...
//     void async_hash(lt::storage_index_t storage, lt::piece_index_t const piece, lt::span<lt::sha256_hash>
//     block_hashes, lt::disk_job_flags_t, std::function<void(lt::piece_index_t, lt::sha1_hash const &,
...
//     void async_hash2(lt::storage_index_t storage, lt::piece_index_t const piece, int const offset,
//     lt::disk_job_flags_t, std::function<void(lt::piece_index_t, lt::sha256_hash const &, lt::storage_error const &)>
//     handler) override
//     {
//         lt::storage_error error;
...
//     void async_move_storage(lt::storage_index_t, std::string p, lt::move_flags_t, std::function<void(lt::status_t,
//     std::string const &, lt::storage_error const &)> handler) override
//     {
//         post(m_ioc, [=]
...
//     void async_delete_files(lt::storage_index_t, lt::remove_flags_t, std::function<void(lt::storage_error const &)>
//     handler) override
//     {
//         post(m_ioc, [=]
//              { handler(lt::storage_error()); });
...
//     void async_check_files(lt::storage_index_t, lt::add_torrent_params const *, lt::aux::vector<std::string,
//     lt::file_index_t>, std::function<void(lt::status_t, lt::storage_error const &)> handler) override
//     {
//         post(m_ioc, [=]
//              { handler(lt::status_t::no_error, lt::storage_error()); });
//     }

//     void async_rename_file(lt::storage_index_t, lt::file_index_t const idx, std::string const name,
...
//     void async_set_file_priority(lt::storage_index_t, lt::aux::vector<lt::download_priority_t, lt::file_index_t>
//     prio, std::function<void(lt::storage_error const &, lt::aux::vector<lt::download_priority_t, lt::file_index_t>)>
//     handler) override
//     {
//         post(m_ioc, [=]
...
//     void async_clear_piece(lt::storage_index_t, lt::piece_index_t index, std::function<void(lt::piece_index_t)>
//     handler) override
//     {
//         post(m_ioc, [=]
//              { handler(index); });
