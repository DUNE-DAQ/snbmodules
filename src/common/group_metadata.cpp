/**
 * @file group_metadata.cpp GroupMetadata class header, used to store the metadata of a group of transfers metadata (one
...
TransferMetadata&
GroupMetadata::add_file(std::shared_ptr<TransferMetadata> meta)
{
...
std::string
GroupMetadata::export_to_string()
{
  nlohmann::json j;
...
void
GroupMetadata::from_string(const std::string& str)
{
...
void
GroupMetadata::generate_metadata_file(std::filesystem::path dest)
{
...
void
GroupMetadata::load_metadata_from_meta_file(std::filesystem::path src)
{
