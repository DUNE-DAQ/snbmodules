...
void
Bookkeeper::create_new_transfer(const std::string& protocol,
                                const std::string& src,
                                const std::set<std::string>& dests,
                                const std::set<std::filesystem::path>& files,
                                const nlohmann::json& protocol_options)
{
  // suppress warnings
  (void)protocol;
  (void)src;
  ...
  //     send_notification(notification_type::e_notification_type::NEW_TRANSFER, get_bookkeeper_id(), session_name,
  //     client, 1000, transfer.export_to_string());
  // }
}

// Only for stand alone application
void
Bookkeeper::input_action(char input)
{
  switch (input) {
    case 'q': {
      TLOG() << "Exiting...";
    ...
    case 'n': {
      ...
      for (int enum_i = protocol_type::e_protocol_type::BITTORRENT; enum_i != protocol_type::e_protocol_type::dummy;
           enum_i++) {
        TLOG() << enum_i << " - "
               << protocol_type::protocols_to_string(static_cast<protocol_type::e_protocol_type>(enum_i));
      }
      int protocol = -1;
      std::cin >> protocol;
      // Check input
      ...
      // create_new_transfer(protocol_type::protocols_to_string(protocol_type::string_to_protocols(protocol).value()),
      // choosen_clients, choosen_files);
      break;
    }
...
Bookkeeper::start_transfers(const std::string& transfer_id)
{
  ...
  if (m_clients_per_grp_transfer.find(transfer_id) != m_clients_per_grp_transfer.end()) {
    for (const std::string& client : m_clients_per_grp_transfer[transfer_id]) {
      ...
      send_notification(
        notification_type::e_notification_type::START_TRANSFER, get_bookkeeper_id(), session_name, client);
    }
  } else {
...
void
Bookkeeper::do_work(std::atomic<bool>& running_flag)
{
  TLOG() << "JAB " << __LINE__;
...
void
Bookkeeper::request_connection_and_available_files(const std::string& client)
{
  // send connection request to client
  send_notification(notification_type::e_notification_type::CONNECTION_REQUEST,
                    get_bookkeeper_id(),
                    client,
                    client,
                    get_bookkeeper_id(),
  ...
  // while (msg.has_value() && msg.value().m_notification !=
  // notification_type::notification_to_string(notification_type::e_notification_type::CONNECTION_REQUEST))
  // {
  //     action_on_receive_notification(msg.value());
...
void
Bookkeeper::request_update_metadata(bool force)
{
  for (const auto& [id, g] : get_grp_transfers()) {
    ...
    if (g.get_group_status() == status_type::e_status::DOWNLOADING ||
        ...
        g.get_group_status() == status_type::e_status::HASHING || force) {

      for (const std::string& session : m_clients_per_grp_transfer[g.get_group_id()]) {
        send_notification(notification_type::e_notification_type::UPDATE_REQUEST,
                          get_bookkeeper_id(),
                          session,
                          get_client_name_from_session_name(session));
      }
    }
  }
}
...
bool
Bookkeeper::action_on_receive_notification(NotificationData notif)
{
  ...
  if (notif.m_target_id.find(get_bookkeeper_id()) == std::string::npos && notif.m_target_id != "all") {
    ers::warning(
      NotificationWrongDestinationError(ERS_HERE, get_bookkeeper_id(), notif.m_source_id, notif.m_target_id));
    return false;
  }
  ...
  if (action.has_value() == false) {
    ers::warning(
      InvalidNotificationReceivedError(ERS_HERE, get_bookkeeper_id(), notif.m_source_id, notif.m_notification));
  }
...
void
Bookkeeper::display_information()
{
  ...
  if (m_file_log_path != "") {
    ...
    TLOG() << "debug : output log wroten " << m_file_log_path << get_bookkeeper_id() << ".log\t" << m_file_log_path
           << get_bookkeeper_id() << "_line.csv";

    // if csv file empty, write header
    if (dynamic_cast<std::ofstream*>(output_line_log)->tellp() == 0) {
      *output_line_log << "time" << sep << "file_name" << sep << "file_full_path" << sep << "group_id" << sep
                       << "src_ip" << sep << "size" << sep

                       << "dest_ip" << sep << "start_time" << sep << "duration" << sep << "progress" << sep << "speed"
                       << sep << "state" << sep
  ...
  *output << "***** Bookkeeper " << get_bookkeeper_id() << " " + get_ip().get_ip_port() << " informations display *****"
          << std::endl;
  ...
  *output << "Connected clients :" << std::endl;
  ...
  for (const auto& client : get_transfers()) {
    ...
    for (const auto& file : client.second) {
      if (m_file_log_path != "") {
        *output_line_log << std::chrono::duration_cast<std::chrono::seconds>(
                              std::chrono::system_clock::now().time_since_epoch())
                              .count()
                         << sep << file->get_file_name() << sep << file->get_file_path() << sep << file->get_group_id()
                         << sep << file->get_src().get_ip_port() << sep << file->get_size() << sep

                         << file->get_dest().get_ip_port() << sep << file->get_start_time_str() << sep
                         << file->get_total_duration_ms() << sep << file->get_progress() << sep
                         << file->get_transmission_speed() << sep << status_type::status_to_string(file->get_status())
                         << sep

                         << file->get_end_time_str() << sep << file->get_error_code() << sep
      ...
      if (is_session)
        *output << file->get_file_name() << "\t" << file->get_size() << " bytes\tfrom " << file->get_src().get_ip_port()
                << "\t"
                // << file->get_dest().get_ip_port() << "\t"
                << status_type::status_to_string(file->get_status()) << "\t" << file->get_progress() << "%\t"
                << (file->get_transmission_speed() == 0 ? "-" : std::to_string(file->get_transmission_speed()))
                << "Bi/s\t" << file->get_start_time_str() << "\t" << file->get_total_duration_ms() << "ms\t"
                << file->get_end_time_str() << "\t" << file->get_error_code() << "\t" << std::endl;
      else
        *output << "Available file " << file->get_file_path() << "\t" << file->get_size() << " bytes\tfrom "
                << file->get_src().get_ip_port() << "\t" << std::endl;
    }
  }

  *output << std::endl << "Active Transfers :" << std::endl;

  *output << "Group ID\t" << "Protocol\t" << "Src\t" << "IP\t" << "Status\t" << std::endl;

  for (const auto& [id, g] : get_grp_transfers()) {
    *output << g.get_group_id() << "\t" << protocol_type::protocols_to_string(g.get_protocol()) << "\t"
            << g.get_source_id() << "\t" << g.get_source_ip().get_ip_port() << "\t"
            << status_type::status_to_string(g.get_group_status()) << "\t" << std::endl;

    for (const std::shared_ptr<TransferMetadata>& fmeta : g.get_transfers_meta()) {
      *output << "\t- " << fmeta->get_file_name() << "\t" << fmeta->get_src().get_ip_port() << " to "
              << fmeta->get_dest().get_ip_port() << "\t" << status_type::status_to_string(fmeta->get_status()) << "\t"
              << std::endl;
    }

    for (const std::string& f : g.get_expected_files()) {
      *output << "\t- " << f << "\t" << "Expected" << std::endl;
    }
  }
...
void
Bookkeeper::add_update_grp_transfer(GroupMetadata grp_transfers)
{
