...
TransferClient::TransferClient(const IPFormat& listening_ip,
...
bool
TransferClient::start(int timeout)
{
...
bool
TransferClient::do_work(std::atomic<bool>& running_flag)
{
...
void
TransferClient::create_new_transfer(const std::string& transfer_id,
                                    ...
                                    const nlohmann::json& protocol_options /*= nlohmann::json()*/)
{
  ...
  for (auto& bk : get_bookkeepers_conn()) {
    send_notification(
      notification_type::e_notification_type::GROUP_METADATA, session_name, bk, bk, group_transfer.export_to_string());
  }
  ...
  for (const auto& client : dest_clients) {
    ...
    send_notification(notification_type::e_notification_type::NEW_TRANSFER,
                      get_client_id(),
                      new_session_name,
                      client,
    ...
    for (auto file : s.get_transfer_options().get_transfers_meta()) {
      send_notification(notification_type::e_notification_type::TRANSFER_METADATA,
                        ...
                        new_session_name,
                        client,
                        file->export_to_string());
    }
  }
}

void
TransferClient::start_transfer(const std::string& transfer_id)
{
...
void
TransferClient::pause_transfer(const std::string& transfer_id)
{
...
void
TransferClient::resume_transfer(const std::string& transfer_id)
{
...
void
TransferClient::cancel_transfer(const std::string& transfer_id)
{
...
TransferSession*
TransferClient::get_session(std::string transfer_id)
{
...
void
TransferClient::share_available_files(const std::set<std::filesystem::path>& to_share, const std::string& dest)
{
  ...
  for (const std::filesystem::path& f : to_share) {
    ...
    send_notification(notification_type::e_notification_type::TRANSFER_METADATA,
                      ...
                      dest,
                      dest,
                      fmeta->export_to_string());
  }
}

bool
TransferClient::action_on_receive_notification(NotificationData notif)
{
  ...
  switch (action.value()) {
    ...
    case notification_type::e_notification_type::NEW_TRANSFER: {
      ...
      TLOG() << "debug : creating session " << notif.m_target_id << " type "
             << TransferSession::session_type_to_string(type);
    ...
    case notification_type::e_notification_type::UPDATE_REQUEST: {
      ...
      if (ses != nullptr) {
        ...
        if (notif.m_data == "") {
          ...
          ses->update_metadata_to_bookkeeper(
            ses->get_transfer_options().get_transfer_meta_from_file_path(notif.m_data));
        }
      } else {
        ers::warning(SessionIDNotFoundInClientError(ERS_HERE, get_client_id(), notif.m_target_id));
      }
...
void
TransferClient::remove_session(const std::string& session_id)
{
...
std::string
TransferClient::generate_session_id(const std::string& transferid, const std::string& dest_id /*= ""*/)
{
  std::string id = "";
...
void
TransferClient::scan_available_files(std::set<std::filesystem::path>& previous_scan,
                                     bool nested,
                                     std::filesystem::path folder) // NOLINT
{
  if (folder.empty()) {
      ...
      if (previous_scan.insert(entry.path()).second) {
        ...
        if (!already_active) {
          ...
          create_session(metadata,
                         Uploader,
                         generate_session_id(group_id_tmp),
                         get_listening_dir().append("ses" + std::to_string(m_sessions.size())));
        }
      }
    }
  }
...
std::shared_ptr<TransferMetadata>
TransferClient::create_metadata_from_file(const std::filesystem::path& src)
{
  return std::make_shared<TransferMetadata>(src, std::filesystem::file_size(src), get_ip());
}

std::string
TransferClient::get_my_conn()
{
  ...
  if (m_my_conn.empty()) {
    for (const std::string& c : get_clients_conn()) {
      ...
      if (str1.starts_with("snbmodules_")) {
        str1 = str1.substr(11);
      }
      if (str2.starts_with("snbmodules_")) {
        str2 = str2.substr(11);
      }

      if (str1.starts_with("snb-sample-config-")) {
        str1 = str1.substr(18);
      }
      if (str2.starts_with("snb-sample-config-")) {
        str2 = str2.substr(18);
      }

      TLOG() << "BBB " << __LINE__ << " c=\"" << c << "\" client_id=" << get_client_id() << " " << str1 << " " << str2;
