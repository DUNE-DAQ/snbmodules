...
/**
 * @file transfer_session.cpp TransferSession class, wrapper to get access to the transfer interface and control states
 * of transfers
 *
...
TransferSession::TransferSession(GroupMetadata transfer_options,
                                 e_session_type type,
                                 std::string id,
                                 const IPFormat& ip,
  ...
  m_work_dir(std::move(work_dir))
{
  ...
  switch (m_transfer_options.get_protocol()) {
    case protocol_type::BITTORRENT:
      ...
      m_transfer_interface = std::make_unique<TransferInterfaceBittorrent>(
        m_transfer_options, type == e_session_type::Downloader, get_work_dir(), get_ip());
      ...
      if (type == e_session_type::Uploader) {
        ...
        for (auto f_meta : m_transfer_options.get_transfers_meta()) {
          ...
          f_meta->set_magnet_link(lt::make_magnet_uri(t) + "&x.pe=" + get_ip().get_ip() + ":" +
                                  m_transfer_options.get_protocol_options()["port"].get<std::string>());
        }
      }
      break;
    ...
    case protocol_type::SCP:
      m_transfer_interface =
        std::make_unique<TransferInterfaceSCP>(m_transfer_options, type == e_session_type::Uploader);
      break;
    ...
    default:
      ers::error(InvalidProtocolError(
        ERS_HERE, get_session_id(), protocol_type::protocols_to_string(m_transfer_options.get_protocol())));
      break;
  }
...
std::string
TransferSession::to_string() const
{
  std::string str;
...
bool
TransferSession::update_metadatas_to_bookkeeper()
{
  bool result = true;
  for (const std::string& bk : get_bookkeepers_conn()) {
    result = result && send_notification(notification_type::e_notification_type::GROUP_METADATA,
                                         get_session_id(),
                                         bk,
                                         bk,
...
bool
TransferSession::update_metadata_to_bookkeeper(TransferMetadata& f_meta)
{
  bool result = true;
  for (const std::string& bk : get_bookkeepers_conn()) {
    result = result && send_notification(notification_type::e_notification_type::TRANSFER_METADATA,
                                         get_session_id(),
                                         bk,
                                         bk,
                                         f_meta.export_to_string_partial(false));
  }
  return result;
}

bool
TransferSession::send_notification_to_targets(notification_type::e_notification_type type,
...
bool
TransferSession::start_file(TransferMetadata& f_meta)
{
  bool res = false;
  if (is_downloader()) {
...
bool
TransferSession::pause_file(TransferMetadata& f_meta, bool is_multiple)
{
  if (f_meta.get_status() != status_type::e_status::DOWNLOADING &&
      f_meta.get_status() != status_type::e_status::UPLOADING) {
    ers::warning(SessionWrongStateTransitionError(ERS_HERE,
                                                  get_session_id(),
                                                  f_meta.get_file_name(),
                                                  status_type::status_to_string(f_meta.get_status()),
                                                  status_type::status_to_string(status_type::e_status::PAUSED)));
    return false;
  }
...
bool
TransferSession::resume_file(TransferMetadata& f_meta, bool is_multiple)
{
...
bool
TransferSession::hash_file(TransferMetadata& f_meta, bool is_multiple)
{
  if (f_meta.get_status() != status_type::e_status::FINISHED) {
    ers::warning(SessionWrongStateTransitionError(ERS_HERE,
                                                  ...
                                                  f_meta.get_file_name(),
                                                  status_type::status_to_string(f_meta.get_status()),
                                                  status_type::status_to_string(status_type::e_status::HASHING)));
    return false;
  }
...
bool
TransferSession::cancel_file(TransferMetadata& f_meta, bool is_multiple)
{
  if (f_meta.get_status() == status_type::e_status::FINISHED ||
      f_meta.get_status() == status_type::e_status::CANCELLED) {
    ers::warning(SessionWrongStateTransitionError(ERS_HERE,
                                                  get_session_id(),
                                                  f_meta.get_file_name(),
                                                  status_type::status_to_string(f_meta.get_status()),
                                                  status_type::status_to_string(status_type::e_status::CANCELLED)));
    return false;
  }
...
bool
TransferSession::upload_file(TransferMetadata& f_meta, bool is_multiple)
{
  ...
  if (f_meta.get_status() != status_type::e_status::WAITING) {
    ers::warning(SessionWrongStateTransitionError(ERS_HERE,
                                                  ...
                                                  f_meta.get_file_name(),
                                                  status_type::status_to_string(f_meta.get_status()),
                                                  status_type::status_to_string(status_type::e_status::UPLOADING)));
    return false;
  }
...
bool
TransferSession::download_file(TransferMetadata& f_meta, std::filesystem::path dest, bool is_multiple)
{
  ...
  if (f_meta.get_status() != status_type::e_status::WAITING) {
    ers::warning(SessionWrongStateTransitionError(ERS_HERE,
                                                  ...
                                                  f_meta.get_file_name(),
                                                  status_type::status_to_string(f_meta.get_status()),
                                                  status_type::status_to_string(status_type::e_status::DOWNLOADING)));
    return false;
  }
...
// Downloaders only
bool
TransferSession::download_all(const std::filesystem::path& dest)
{
  if (m_type != e_session_type::Downloader) {
