/**
 * @file notification_interface.hpp NotificationData class, NotificationInterface class, interface used by clients or
 * bookkeepers to send/receive notifications
 *
...
struct NotificationData
{
  ...
  NotificationData(std::string source_id, std::string target_id, std::string notification, std::string data = "")
    ...
    , m_target_id(std::move(target_id))
    , m_notification(std::move(notification))
    , m_data(std::move(data))
  {
  }
  NotificationData(std::string source_id,
                   std::string target_id,
                   notification_type::e_notification_type notification,
                   std::string data = "")
    : m_source_id(std::move(source_id))
    , m_target_id(std::move(target_id))
    , m_notification(notification_type::notification_to_string(notification))
    , m_data(std::move(data))
  {
  }
...
public:
  explicit NotificationInterface(std::string connection_prefix = "snbmodules",
                                 int timeout_send = 10,
  ...
  NotificationInterface(std::vector<std::string> bk_conn,
  ...
  std::optional<NotificationData> listen_for_notification(const std::string& id,
  ...
  /// @return
  bool send_notification(const notification_type::e_notification_type& notif,
  ...
  // TEMP_REMOVED//void init_connection_interface(const std::string &session_name = "SNBMODULES", bool
  // use_connectivity_service = false, const IPFormat &ip = IPFormat("localhost", 5000)) TEMP_REMOVED//{
