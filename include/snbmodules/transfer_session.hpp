/**
 * @file transfer_session.hpp e_session_type enum downloader or uploader, TransferSession class, wrapper to get access
 * to the transfer interface and control states of transfers
 *
...
public:
  ...
  TransferSession(GroupMetadata transfer_options,
                  e_session_type type,
                  std::string id,
                  const IPFormat& ip,
  ...
  {
    const std::map<e_session_type, std::string> MyEnumStrings{ { Downloader, "Downloader" }, { Uploader, "Uploader" } };
