/**
 * @file group_metadata.hpp GroupMetadata class header, used to store the metadata of a group of transfers metadata (one
 * uploader to multiple downloaders)
 *
...
public:
  ...
  /// @param transfer_id
  GroupMetadata(std::string group_id,
                std::string source,
                const IPFormat& source_ip,
                protocol_type::e_protocol_type protocol,
                const nlohmann::json& protocol_options = nlohmann::json(),
                const std::vector<std::shared_ptr<TransferMetadata>>& transfers_meta =
                  std::vector<std::shared_ptr<TransferMetadata>>())
  ...
  inline void set_transfers_meta(std::vector<std::shared_ptr<TransferMetadata>> files_meta)
  {
    m_transfers_meta = std::move(files_meta);
  }
  inline void set_protocol_options(nlohmann::json protocol_options)
  {
    m_protocol_options = std::move(protocol_options);
  }
