...
public:
  ...
  /// @param listening_dir Listening directory (where the client will save incoming files and files to share with
  /// bookkeepers)
  TransferClient(const IPFormat& listening_ip,
  ...
  void create_new_transfer(const std::string& transfer_id,
  ...
  TransferSession& create_session(GroupMetadata transfer_options,
                                  e_session_type type,
                                  std::string id,
  ...
  void scan_available_files(std::set<std::filesystem::path>& previous_scan,
                            bool nested = false,
