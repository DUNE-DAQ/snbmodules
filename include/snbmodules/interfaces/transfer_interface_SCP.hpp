...
public:
  ...
  bool download_file(TransferMetadata& f_meta, std::filesystem::path dest) override
  {
    ...
    if (m_params.use_password) {
      exec = "scp " + m_params.user + "@" + f_meta.get_src().get_ip() + ":" + f_meta.get_file_path().string() + " " +
             dest.string();
    } else {

      exec = "scp -o PasswordAuthentication='no' " + m_params.user + "@" + f_meta.get_src().get_ip() + ":" +
             f_meta.get_file_path().string() + " " + dest.string();
    }
