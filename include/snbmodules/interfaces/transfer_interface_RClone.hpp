...
public:
  ...
  bool upload_file(TransferMetadata& f_meta) override
  {
    ...
    // char exec[300];
    // sprintf(exec,
    //         "rclone serve http / --addr %s:%d --buffer-size '%s' --no-modtime --transfers %d -vv --vfs-cache-mode
    //         'off' --vfs-cache-max-size 'off'", f_meta.get_src().get_ip().c_str(), m_params.port,
  ...
  bool download_file(TransferMetadata& f_meta, std::filesystem::path dest) override
  {
    ...
    if (m_params.protocol == "http") {
      std::string file_relative_path =
        std::filesystem::relative(f_meta.get_file_path(), m_params.root_folder).generic_string();
