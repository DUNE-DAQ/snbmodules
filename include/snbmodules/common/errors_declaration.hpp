...
ERS_DECLARE_ISSUE(snbmodules,
                  ...
                  "NotificationWrongDestinationError: " << location << " Received notification from " << source
                                                        << " to " << dest,
                  ((std::string)location)((std::string)source)((std::string)dest)) // NOLINT

ERS_DECLARE_ISSUE(snbmodules,
                  InvalidNotificationReceivedError,
                  "InvalidNotificationReceivedError: " << location << " Received invalid notification " << notif
                                                       << " from " << source,
                  ((std::string)location)((std::string)source)((std::string)notif)) // NOLINT

ERS_DECLARE_ISSUE(snbmodules,
                  NotHandledNotificationError,
                  "NotHandledNotificationError: " << location << " Received notification " << notif << " from "
                                                  << source << " but theire is no action for it !",
                  ((std::string)location)((std::string)source)((std::string)notif)) // NOLINT
...
ERS_DECLARE_ISSUE(
  ...
  "SessionTypeNotSupportedError: trying to create a session with incorect type Downloader/Uploader for session: "
    << session,
  ((std::string)session)) // NOLINT

ERS_DECLARE_ISSUE(snbmodules,
                  SessionAccessToIncorrectActionError,
                  "SessionAccessToIncorrectActionError: session " << session << " try to access to " << action
...
ERS_DECLARE_ISSUE(
  ...
  "SessionWrongStateTransitionError: " << location << " session try to change transition from " << current_state
                                       << " to " << future_state << " for file " << file,
...
ERS_DECLARE_ISSUE(snbmodules,
                  ...
                  "InvalidSourceCommandRequestError: " << command
...
ERS_DECLARE_ISSUE(
  ...
  "ErrorSCPDownloadError: An Error happend while executing the SCP transfer command. The file have not been send. "
    << error_msg,
...
ERS_DECLARE_ISSUE(snbmodules, BittorrentError, "BittorrentError: " << error_msg,
                  ((std::string)error_msg)) // NOLINT

ERS_DECLARE_ISSUE(snbmodules,
                  BittorrentInvalidMagnetLinkError,
                  "BittorrentInvalidMagnetLinkError: Unable to add magnet link file to bittorent session : "
                    << error_msg,
                  ((std::string)error_msg)) // NOLINT
...
ERS_DECLARE_ISSUE(snbmodules,
                  ...
                  "MetadataNotFoundInGroupError: Didn't found metadata object in "
                    << group << " group for specified file path : " << path,
...
ERS_DECLARE_ISSUE(
  ...
  "MetadataReadingWrongKeyValueDataError: Reading empty or unknown key/value pair entry for metadata file : k="
    << key << " v=" << value,
