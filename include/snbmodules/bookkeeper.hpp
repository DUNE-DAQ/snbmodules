...
public:
  ...
  Bookkeeper(const IPFormat& listening_ip,
  ...
  [[deprecated("Now only the uploader can create a new transfer")]] void create_new_transfer(
  ...
  /// @brief Display the information of the bookkeeper either on the normal log or on a specific file depending on the
  /// value of m_file_log_path
  ...
  inline const std::map<std::string, std::vector<std::shared_ptr<TransferMetadata>>>& get_transfers() const
  {
    return m_transfers;
  }
