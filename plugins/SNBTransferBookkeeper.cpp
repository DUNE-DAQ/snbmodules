/**
 * @file SNBTransferBookkeeper.cpp SNBTransferBookkeeper Bookkeeper module retriving transfers informations from
 * SNBFileTransfer clients.
 *
...
{
  m_bookkeeper = std::make_shared<Bookkeeper>(IPFormat(m_snbbk_conf->get_bookkeeper_ip()),
                                              m_name,
                                              m_snbbk_conf->get_bookkeeper_log_path(),
                                              m_snbbk_conf->get_refresh_rate(),
                                              m_snbbk_conf->get_connection_prefix(),
                                              m_snbbk_conf->get_timeout_send(),
                                              m_snbbk_conf->get_timeout_receive());
  m_thread = std::make_unique<dunedaq::utilities::WorkerThread>(
    [&](std::atomic<bool>& running) { m_bookkeeper->do_work(running); });
}
