...
SNBFileTransfer::do_tr_new(const CommandData_t& args)
{
  ...
  if (src == m_name) {
    ...
    m_client->create_new_transfer(
      args["transfer_id"].get<std::string>(), args["protocol"].get<std::string>(), dests, files, args["protocol_args"]);
  } else {
...
SNBFileTransfer::do_conf(const CommandData_t& /*payload*/)
{
  ...
  m_client = std::make_shared<TransferClient>(IPFormat(m_snbft_conf->get_client_ip()),
                                              m_name,
                                              m_snbft_conf->get_work_dir(),
                                              m_snbft_conf->get_connection_prefix(),
                                              m_snbft_conf->get_timeout_send(),
                                              m_snbft_conf->get_timeout_receive());
  m_thread =
    std::make_unique<dunedaq::utilities::WorkerThread>([&](std::atomic<bool>& running) { m_client->do_work(running); });
}
