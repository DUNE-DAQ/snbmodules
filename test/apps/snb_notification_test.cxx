      ...
      if (pid == 0) {
        ...
        if (i == n_apps - 1) {
          ...
          for (int j = 0; j < n_apps - 1; j++) {
            bookkeeper.add_connection(
              IPFormat("localhost", starting_port + j), "client" + std::to_string(j), "notification_t", true);
          }
        ...
        // Clients process
          ...
          assert(client.send_notification(notification_type::e_notification_type::CONNECTION_REQUEST,
                                          client.get_client_id(),
                                          "bookkeeper",
                                          "client" + std::to_string(i)) == true);
        }
        return 0;
      } else {
        // Parent process
