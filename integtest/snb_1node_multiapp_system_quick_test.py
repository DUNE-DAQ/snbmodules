...
with open('new-RClone-transfer.json', 'r+') as f:
    ...
    data['data']['modules'][0]['data']['src'] = host_interface+"snbclient00"
    data['data']['modules'][0]['data']['dests'] = [ f"{host_interface}snbclient{i}0" for i in range(1, snb_clients_number)]
    data['data']['modules'][0]['data']['protocol_args']['user'] = sftp_user_name
    data['data']['modules'][0]['match'] = host_interface+"snbclient00"
...
def test_local_transfer_snbmodules(run_nanorc):
    ...
    for i in range(snb_clients_number):
        assert exists(run_nanorc.run_dir / f"{host_interface}snbclient{i}0")
    
    for i in range(number_of_data_producers):
        file_name = "output_" + interface_name + "_" + str(i) + ".out"
        for i in range(1,snb_clients_number):
            assert exists(run_nanorc.run_dir / f"{host_interface}snbclient{i}0/transfer0/" / file_name)
            # Compare file size and content
            assert raw_file_check.compare_raw_size(run_nanorc.run_dir / file_name, run_nanorc.run_dir / f"{host_interface}snbclient{i}0/transfer0/" / file_name)
            assert raw_file_check.compare_raw_content(run_nanorc.run_dir / file_name, run_nanorc.run_dir / f"{host_interface}snbclient{i}0/transfer0/" / file_name)
